...
BASE_CONFIG="${1:-config_stage2_moving.yaml}"
RESUME="${2:-}"
RUN_NAME="${3:-stage2_moving}"
...
PY
  if [ -n "${RESUME}" ]; then
    CUDA_VISIBLE_DEVICES=${GPU} python -m stage2.train_vector --config "${CFG}" --resume "${RESUME}" \
      > "${LOG}" 2>&1 &
  else
    CUDA_VISIBLE_DEVICES=${GPU} python -m stage2.train_vector --config "${CFG}" \
      > "${LOG}" 2>&1 &
  fi
  PID="$!"
  PIDS+=("${PID}")
...
echo "launched 4 stage2 vectorized runs: ${RUN_NAME}"
...
while true; do
  ...
  echo "----- $(date '+%Y-%m-%d %H:%M:%S') latest stage2 training status -----"
  for IDX in "${!LOGS[@]}"; do
    LOG="${LOGS[$IDX]}"
    if [ -s "${LOG}" ]; then
      printf "gpu%s: " "${IDX}"
...
for IDX in "${!PIDS[@]}"; do
  ...
  CODE="$?"
  if [ "${CODE}" -eq 0 ]; then
    echo "gpu${IDX}: stage2 training finished successfully"
  else
    echo "gpu${IDX}: stage2 training failed with exit code ${CODE}; check ${LOGS[$IDX]}"
    STATUS=1
  fi
done
set -e
